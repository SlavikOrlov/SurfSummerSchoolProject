...
//
//  Created by Slava Orlov on 21.08.2022.
//

import Foundation

func phoneMask(phoneNumber: String, shouldRemoveLastDigit: Bool) -> String {
    
    let maxNumberCountInPhoneNumberField = 11
    var regex: NSRegularExpression? {
        do {
            let regexExpression = try NSRegularExpression(
                pattern: "[\\+\\s-\\(\\)]",
    ...
    if number.count > maxNumberCountInPhoneNumberField {
        let maxIndex = number.index(
            number.startIndex,
            offsetBy: maxNumberCountInPhoneNumberField
        )
...
func clearPhoneNumberFromMask(phoneNumber: String) -> String {
    let phoneNumberClearedFromSymbols = phoneNumber
